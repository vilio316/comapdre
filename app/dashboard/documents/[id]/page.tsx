...
export default function DocumentViewerPage() {
  ...
  const [data, setData] = useState<{ url: string; name: string; type: string } | null>(null);
  ...
  if (error || !data) {
    return (
      <div className="mx-auto max-w-xl py-16 text-center">
        <p className="text-lg font-semibold text-deep">Could not load document</p>
        <p className="mt-1 text-sm text-ink-muted">{error ?? "Unknown error"}</p>
        <Link
          href="/dashboard"
    ...
    <div className="flex flex-col pb-8">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="truncate text-lg font-bold text-deep sm:text-xl">{data.name}</h1>
        </div>
        <a
          href={data.url}
          download={data.name}
      ...
      {data.type === "pdf" ? (
        ...
        <div className="rounded-xl border border-gray-200 bg-surface p-10 shadow-sm">
          <div className="mx-auto max-w-md text-center">
            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-deep/5">
              <svg className="h-7 w-7 text-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
              </svg>
            </div>
            <h2 className="text-lg font-semibold text-deep">Preview not available</h2>
        ...
        <div className="rounded-xl border border-gray-200 bg-surface p-10 shadow-sm">
          <div className="mx-auto max-w-md text-center">
            <p className="text-sm text-ink-muted">This file type cannot be previewed.</p>
            <a
              href={data.url}
              download={data.name}
