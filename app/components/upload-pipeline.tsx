...
export default function UploadPipeline({ onComplete, onCancel }: UploadPipelineProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-surface shadow-sm">
      <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
        ...
        <button onClick={onCancel} className="rounded-md p-1 text-ink-muted hover:bg-gray-100 hover:text-deep transition-colors">
      ...
      <div className="p-5">
        {step === 'select' && (
          <div
            ...
            className={`flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-8 transition-colors ${
              dragOver ? 'border-blue bg-blue/5' : error ? 'border-red-300 bg-red-50/30' : 'border-gray-300 hover:border-gold/50'
            }`}
          >
            <input
              ref={inputRef}
            ...
            {file ? (
              <div className="flex w-full items-center gap-4" onClick={(e) => e.stopPropagation()}>
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-deep/5">
                  <svg className="h-6 w-6 text-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                  </svg>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-deep">{file.name}</p>
              ...
              <>
                <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-deep/5">
                  <svg className="h-5 w-5 text-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <p className="mt-1 text-xs text-ink-muted">PDF or DOCX up to 30 MB</p>
              </>
            )}
          </div>
        )}
        ...
        {step === 'uploading' && (
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-deep">{file?.name}</p>
                <p className="text-xs text-ink-muted">{file && formatSize(file.size)}</p>
              </div>
              <span className="text-xs font-medium text-deep">{Math.round(progress)}%</span>
            </div>
        ...
        {step === 'done' && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 rounded-lg bg-green-50 p-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-green-800">Uploaded successfully</p>
                <p className="truncate text-xs text-green-600">{file?.name}</p>
              </div>
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-medium text-ink-muted">Add tags</label>
              <div className="flex flex-wrap gap-1.5 mb-2">
                {tags.map((t) => (
                  <span key={t} className="inline-flex items-center gap-1 rounded-full bg-gold/10 px-2.5 py-0.5 text-xs font-medium text-gold">
                    {t}
                    <button onClick={() => setTags(tags.filter((x) => x !== t))} className="text-gold/60 hover:text-gold">
        ...
        {step === 'done' && (
          <div className="mt-4 flex gap-2">
            ...
            <button
              onClick={() => { setStep('select'); setFile(null); setProgress(0); setTags([]); setTagInput('') }}
              className="flex-1 rounded-lg border border-gray-200 py-2.5 text-sm font-medium text-ink-muted transition-colors hover:bg-gray-50"
            >
              Upload another
            </button>
